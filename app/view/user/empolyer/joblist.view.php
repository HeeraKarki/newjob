        ...
        <div class="job-profile section" style="box-shadow: 2px 9px 7px rgba(249,219,255,0.43);">
            <div class="user-profile">
                <div class="user-images">
                    <!--                    <img src="-->
                    <? //= asset('applyjob/')?><!--images/user.jpg" alt="User Images" class="img-responsive">-->
                    <img width="100"
                         src="<?= isset($user->employer->avatar) ? $user->employer->avatar !== NULL ? asset($user->employer->avatar) : '' : asset('applyjob/images/user.jpg') ?>"
                         alt="User Images" class="img-responsive">
                </div>
                <div class="user">
                ...
                <div class="favorites-user">
                    <div class="favorites">
                        <a href="bookmark.php"><?= $user->employer->post_count ?>
                            <small>Post Left</small>
                        </a>
                    </div>
                </div>
            </div><!-- user-profile -->
            ...
            <ul class="user-menu">
                ...
                <li class="active"><a href="<?= baseurl('Employer/Job_List') ?>">My Job List</a></li>
                    ...
                    <div class="item-info">
                        <div class="item-image-box">
                            <div class="item-image">
                                <a href="<?= baseurl('Job_Detail') . "?job_id=" . $job_post->id ?>"><img
                                        src="<?= asset($job_post->employer->logo) ?>" alt="Image"
                        ...
                        <div class="ad-info">
                            ...
                            <div class="ad-meta">
                                <ul>
                                    <li><a href="<?= baseurl('Job_Detail') . "?job_id=" . $job_post->id ?>"><i
                                                class="fa fa-map-marker"
                                                aria-hidden="true"></i><?= $job_post->location->name ?> </a></li>
                                    <li><a href="<?= baseurl('Job_Detail') . "?job_id=" . $job_post->id ?>"
                                           class="text-capitalize"><i class="fa fa-clock-o"
                                                                      aria-hidden="true"></i><?= $job_post->salary_type ?>
                                        </a></li>
                                    <li><a href="<?= baseurl('Job_Detail') . "?job_id=" . $job_post->id ?>"><i
                                                class="fa fa-money"
                                                aria-hidden="true"></i>$<?= number_format($job_post->salary_min) ?>-
                                            $<?= number_format($job_post->salary_max) ?></a></li>
                                </ul>
                            </div><!-- ad-meta -->
                        </div><!-- ad-info -->
                    </div><!-- item-info -->
                            ...
                            <?php if ($applicant->status === "pending"): ?>
                                <tr>
                                    <td class="text-center"><a
                                            href="<?= baseurl('Seeker_Profile') . "?name=" . str_slug($applicant->job_seeker->fullname) ?>"
                                            class="text-capitalize"><?= $applicant->job_seeker->fullname ?></a></td>
                                    <td class="text-center"><?= date('d M Y', strtotime($applicant->created_at)) ?></td>
                                    <form action="<?= baseurl('Employer/Interview') ?>" method="post">

                                        <td>
                                            <input type="date" data-format="d-m-Y" name="interview_date">
                                            <input type="hidden" name="applicant_id" value="<?= $applicant->id; ?>">
                                        </td>
                                        <td class="text-center">
                                            <a title="Reject the applicant"
                                               href="<?= baseurl('Seeker_Profile') . "?name=" . str_slug($applicant->job_seeker->fullname) ?>"
                                               class="btn btn-info">
                                            ...
                                            <a href="<?= baseurl('Employer/Reject') . '?applicant_id=' . $applicant->id ?>"
                            ...
                            <?php if ($applicant->status === "interview"): ?>
                                <tr>
                                    <td class="text-center"><a
                                            ...
                                            class="text-capitalize"><?= $applicant->job_seeker->fullname ?></a></td>
                                    <td class="text-center"><?= date('d M Y', strtotime($applicant->created_at)) ?></td>
                                    <td class="text-center"><?= date('d M Y', strtotime($applicant->interview_date)) ?></td>
                                    <td class="text-center">
                                        <a href="<?= baseurl('Employer/Interview_pass') . '?applicant_id=' . $applicant->id ?>" type="submit" class="btn btn-success">
                                            <i class="fa fa-vcard"></i> Interview Pass
                                        </a>
                                        <a href="<?= baseurl('Employer/Interview_fail') . '?applicant_id=' . $applicant->id ?>"
                                           class="btn btn-danger">
                                            <i class="fa fa-user-times"></i> Interview Fail
                                        </a>
                                    </td>
                                </tr>
